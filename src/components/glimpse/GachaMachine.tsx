...
export default function GachaMachine({
  ...
  return (
    <div className="flex flex-col items-center mb-12 py-10">
      <div
        className={`relative w-48 h-64 bg-zinc-100 dark:bg-zinc-800 border-4 border-zinc-200 dark:border-zinc-700 rounded-3xl flex flex-col items-center justify-between p-4 shadow-xl overflow-hidden ${
      ...
      >
        ...
        {/* The Crank */}
        <div
          className={`w-16 h-16 bg-zinc-200 dark:bg-zinc-700 rounded-full border-4 border-zinc-300 dark:border-zinc-600 flex items-center justify-center transform transition-transform duration-500 ${
            isJumping ? "rotate-180" : ""
          }`}
        >
        ...
        <div className="w-24 h-8 bg-zinc-900 rounded-t-lg border-x-4 border-t-4 border-zinc-700 mt-2 flex items-center justify-center">
          <div
            className={`w-6 h-6 rounded-full transition-all duration-500 ${
              isJumping ? "bg-indigo-400 shadow-[0_0_15px_rgba(129,140,248,0.8)]" : "bg-transparent"
            }`}
          ></div>
        </div>

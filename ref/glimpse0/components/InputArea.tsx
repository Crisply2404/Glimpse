...
const InputArea: React.FC<InputAreaProps> = ({ onStart, onClear, onReplay, isProcessing }) => {
  ...
  return (
    <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
      ...
      <textarea
        ...
        placeholder="e.g. A game about flying islands with anime art style, released around 2010..."
        value={description}
      ...
      <div className="mt-6">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-zinc-500 mb-3">Clue Management</h3>
        <div className="flex flex-wrap gap-2 mb-4">
          {clues.map((clue) => (
            ...
            >
              <i className={`fa-solid ${clue.direction === ClueDirection.INCLUDE ? 'fa-plus' : 'fa-minus'} text-[10px]`}></i>
              <span>{clue.text}</span>
              <span className="opacity-60 text-[10px]">Lvl.{clue.strength}</span>
              <button onClick={() => removeClue(clue.id)} className="hover:text-zinc-900 dark:hover:text-white transition-colors">
                <i className="fa-solid fa-xmark"></i>
              </button>
            </div>
          ))}
          {clues.length === 0 && <p className="text-sm text-zinc-400 italic">No specific clues added yet.</p>}
        </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg overflow-hidden">
              <button 
                ...
                className={`px-3 py-2 text-xs transition-colors ${newClueDirection === ClueDirection.INCLUDE ? 'bg-green-500 text-white' : 'hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}
              >
                Include
              </button>
              <button 
                onClick={() => setNewClueDirection(ClueDirection.EXCLUDE)}
                className={`px-3 py-2 text-xs transition-colors ${newClueDirection === ClueDirection.EXCLUDE ? 'bg-red-500 text-white' : 'hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}
              >
                Exclude
              </button>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] text-zinc-500 uppercase font-bold text-center">Power: {newClueStrength}</span>
              <input 
      ...
      <div className="mt-8 flex gap-3 border-t border-zinc-200 dark:border-zinc-800 pt-6">
        ...
        <button 
          onClick={onReplay}
          disabled={isProcessing || clues.length === 0}
          className="px-6 border border-zinc-200 dark:border-zinc-700 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
        >
          Replay
        </button>
